...
namespace Project1.Services.ContactServices
{
    public class ContactService:IContactService
    {
        ...
        public async Task<ServiceResponse<GetContactDto>> AddNew(AddContactDto newContact)
        {
            ...
            var addedContact=_mapper.Map<Contact>(newContact);

            try
                {
                    addedContact.Id=Contacts.Max(c=>c.Id)+1;
                    Contacts.Add(addedContact);
                    serviceResponse.Data=_mapper.Map<GetContactDto>(addedContact);
                    serviceResponse.Message=$"User {addedContact.Name} successfully added.";
